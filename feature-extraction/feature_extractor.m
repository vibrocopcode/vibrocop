...
%saved start value is the call recieve time after accelerometer recording
savedStartValue = [5,18,19,17,32,14,13,10,3,3];
%[numbers, strings, raw] = xlsread('acctest.xls');
%global nextRow;
%nextRow = size(raw, 1);

%the loop created for handling 10 robocalls at once, it can be configured.

for i = 1:10

    filename = ['humancall/independent_data/auth_ind_p5_v', num2str(i), '.csv']; 
    fnameString="test"; 
    fnameNeumericStart=1; 

    class="robo"; % 'robo'= robocall and 'human' = humancall
    ...
    for x=1:xlimit
        ...
        savedEnd=savedStart+10;
        disp(x);
        my_xls(filename,savedStart,savedEnd,endVal,class,num,Fs);
...
function my_xls(filename,start,funcend,endVal,class,num,Fs)
...
% Assuming 'signal' is  data vector
